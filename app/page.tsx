...
export default function HomePage() {
  ...
  const testimonials = [
    ...
    { quote: "Finally someone who makes business news feel relevant to my life.", handle: "@hustlermark_" },
  ...
  const categories = [
    { name: "Politics", slug: "politics", count: categoryStats.politics || 0, color: "#ef4444", bg: "rgba(239,68,68,0.08)", desc: "Breaking down political events and what they actually mean for you." },
    { name: "Business", slug: "business", count: categoryStats.business || 0, color: "#3b82f6", bg: "rgba(59,130,246,0.08)", desc: "From startup economics to Wall Street — explained for real people." },
    { name: "Culture", slug: "culture", count: categoryStats.culture || 0, color: "#8b5cf6", bg: "rgba(139,92,246,0.08)", desc: "Parasocial relationships, internet trends and what they reveal." },
    { name: "Personal", slug: "personal", count: categoryStats.personal || 0, color: "#10b981", bg: "rgba(16,185,129,0.08)", desc: "Burnout, imposter syndrome, and the conversations creators avoid." },
  ]
  ...
  return (
    <div style={{ background: "var(--bg)" }}>
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" style={{ paddingTop: "7rem", paddingBottom: "5rem" }}>
          <div className="max-w-2xl">
            ...
            <h1 className="mb-5 fade-up" style={{
              ...
              Get Smarter<br />Every Morning.<br /><span className="grad-blue">For Free.</span>
            </h1>
            <p className="mb-8 max-w-sm" style={{ fontSize: "1.1rem", lineHeight: 1.75, color: "var(--text-secondary)" }}>
              Charles Peralo&apos;s daily newsletter turns today&apos;s biggest headlines into something you actually understand and remember.
            </p>
            <div
              className="rounded-2xl p-6 sm:p-8 mb-8 relative overflow-hidden"
              style={{ background: "var(--bg-card)", border: "1px solid var(--border-bright)", boxShadow: "0 0 0 1px rgba(59,130,246,0.1), 0 32px 64px rgba(0,0,0,0.4)", maxWidth: "480px" }}
            >
              ...
              <div className="mb-5">
                <div className="font-display text-xl sm:text-2xl mb-1.5" style={{ color: "var(--text-primary)" }}>Join the Daily Briefing</div>
                <p className="text-sm" style={{ color: "var(--text-muted)" }}>Free. Delivered every morning. Unsubscribe anytime.</p>
              </div>
              <NewsletterForm variant="hero" />
              <div className="mt-6 pt-5" style={{ borderTop: "1px solid var(--border)" }}>
                <div className="text-xs mb-3 font-semibold tracking-wider uppercase" style={{ color: "var(--text-muted)" }}>What&apos;s inside every edition</div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 text-center">
            {[
              ...
              <div key={stat.label} className="flex flex-col items-center fade-up">
                <div className="stat-num grad-blue">{stat.value}</div>
                <div className="text-xs mt-2 tracking-wider uppercase" style={{ color: "var(--text-muted)" }}>{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="ticker-wrap">
          <div className="ticker-track" style={{ animationDuration: "40s" }}>
            {testimonials.map((t, i) => (
              <div
                key={i}
                className="inline-flex flex-col justify-between mx-3 p-4 rounded-xl flex-shrink-0"
                style={{
                  width: "260px",
                  height: "120px",
              ...
              >
                <div className="flex gap-0.5 mb-2 flex-shrink-0">
                  {[1,2,3,4,5].map((s) => (
                    <Star key={s} className="w-3 h-3 fill-current flex-shrink-0" style={{ color: "#f59e0b" }} />
                  ))}
                </div>
                <p className="text-xs leading-relaxed clamp-2 flex-1" style={{ color: "var(--text-secondary)", overflow: "hidden" }}>&ldquo;{t.quote}&rdquo;</p>
                <span className="text-xs font-semibold mt-2 flex-shrink-0" style={{ color: "var(--text-muted)" }}>{t.handle}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              {featuredPosts.slice(0, 5).map((post, i) => (
                <Link key={post.slug} href={`/article/${post.slug}`} className="block group article-list-item">
                  ...
                  <div>
                    ...
                    <p className="text-sm mt-1.5 clamp-2" style={{ color: "var(--text-muted)", lineHeight: 1.6 }}>{post.excerpt}</p>
                  </div>
                </Link>
              ))}
            </div>
            {featuredPosts[0] && (
              <div className="lg:col-span-7">
                <Link href={`/article/${featuredPosts[0].slug}`} className="block group h-full">
                  <div className="grad-border h-full flex flex-col overflow-hidden" style={{ minHeight: "520px" }}>
                    <div className="relative flex items-center justify-center overflow-hidden flex-1" style={{ background: "linear-gradient(135deg, #080d1a 0%, #100820 100%)", minHeight: "300px" }}>
                      <div className="editorial-num select-none" style={{ fontSize: "clamp(8rem, 22vw, 18rem)" }}>
                      ...
                      <div className="absolute inset-0 flex flex-col justify-between p-6">
                        ...
                        <div style={{ background: "linear-gradient(to top, rgba(13,13,20,1) 0%, transparent 100%)", marginLeft: "-1.5rem", marginRight: "-1.5rem", padding: "1.5rem", paddingTop: "3rem" }}>
                          ...
                          <h3 className="font-display text-2xl sm:text-3xl group-hover:text-blue-400 transition-colors" style={{ color: "var(--text-primary)", lineHeight: 1.2 }}>
                    ...
                    <div className="p-6 flex flex-col gap-4" style={{ background: "var(--bg-card)" }}>
                      <p className="text-sm clamp-3" style={{ color: "var(--text-secondary)", lineHeight: 1.75 }}>{featuredPosts[0].excerpt}</p>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="inline-flex items-center gap-1.5 text-xs font-semibold reveal-arrow" style={{ color: "var(--accent)" }}>
      ...
      {/* ══ GIANT QUOTE ══ */}
      <section className="section-pad overflow-hidden" style={{ background: "var(--bg-card)", borderBottom: "1px solid var(--border)", position: "relative" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-4xl">
            <div className="absolute -top-8 -left-4 font-display-black select-none pointer-events-none" style={{ fontSize: "20rem", lineHeight: 1, color: "rgba(59,130,246,0.04)", fontFamily: "var(--font-playfair), Georgia, serif" }}>
              &ldquo;
            </div>
            ...
            <div className="mt-10 flex items-center gap-6">
              <div className="w-12 h-12 rounded-full overflow-hidden" style={{ border: "2px solid var(--border-bright)" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {categories.map((cat) => (
              <Link key={cat.slug} href={`/category/${cat.slug}`} className="group block">
                <div className="p-6 sm:p-8 rounded-2xl flex items-start gap-5 transition-all duration-200 card-dark" style={{ borderLeft: `3px solid ${cat.color}` }}>
                  <div className="w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center font-display-black text-xl" style={{ background: cat.bg, color: cat.color }}>
                    {cat.name[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-display text-lg group-hover:text-blue-400 transition-colors" style={{ color: "var(--text-primary)" }}>{cat.name}</div>
                      <span className="text-xs" style={{ color: "var(--text-muted)" }}>{cat.count} articles</span>
                    </div>
                    <p className="text-sm clamp-2" style={{ color: "var(--text-secondary)", lineHeight: 1.6 }}>{cat.desc}</p>
                  </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div>
              ...
              <h2 className="section-headline mb-5" style={{ color: "var(--text-primary)" }}>See It In<br />Action</h2>
              <p className="mb-8" style={{ fontSize: "1.05rem", color: "var(--text-secondary)", lineHeight: 1.8 }}>
                Charles breaks down today&apos;s biggest story — turning what could be a confusing headline into something you actually understand and remember.
              </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-end justify-between mb-10">
            <h2 className="section-headline" style={{ color: "var(--text-primary)" }}>
              Think You Know?<br /><span style={{ color: "var(--text-secondary)", fontWeight: 400 }}>Prove It.</span>
            </h2>
            ...
            {[
              ...
              <div key={quiz.title} className="grad-border overflow-hidden group">
                <div className="p-7 sm:p-8 flex flex-col gap-5" style={{ background: "var(--bg-card)" }}>
                  ...
                  <h3 className="font-display text-xl sm:text-2xl" style={{ color: "var(--text-primary)", lineHeight: 1.2 }}>{quiz.title}</h3>
      ...
      {/* ══ BOTTOM CTA ══ */}
      <section className="relative overflow-hidden section-pad" style={{ background: "var(--bg)" }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <h2 className="mb-4" style={{
            ...
            Start Knowing<br /><span className="grad-blue">More</span> Tomorrow
          </h2>
          <p className="mb-10 max-w-lg mx-auto" style={{ fontSize: "1.1rem", color: "var(--text-secondary)", lineHeight: 1.75 }}>
            50,000+ readers get Charles&apos;s daily briefing every morning. Free. No fluff. Just the insights that matter.
          </p>
