...
export function Header() {
  ...
  return (
    <header
      style={{
        background: scrolled ? "rgba(13,13,20,0.98)" : "rgba(13,13,20,0.88)",
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <nav className="hidden lg:flex items-center gap-0.5">
            ...
            <div className="relative" onMouseEnter={() => setIsArticlesOpen(true)} onMouseLeave={() => setIsArticlesOpen(false)}>
              <button className="nav-link flex items-center gap-1 px-4 py-2 text-sm font-medium rounded-lg hover:bg-white/5 transition-all">
                Articles <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isArticlesOpen ? "rotate-180" : ""}`} />
              </button>
              {isArticlesOpen && (
                <div className="absolute top-full left-0 mt-2 w-44 rounded-xl py-2 z-50 mobile-nav-enter" style={{ background: "var(--bg-elevated)", border: "1px solid var(--border-bright)", boxShadow: "0 20px 40px rgba(0,0,0,0.5)" }}>
                  {[
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <ThemeToggle />
            <Link href="/subscribe" className="px-5 py-2 rounded-lg text-sm font-semibold btn-accent touch">Subscribe Free</Link>
          </div>

          {/* Mobile actions */}
          <div className="lg:hidden flex items-center gap-1">
            <button onClick={openSearch} aria-label="Search" className="p-2 rounded-lg touch" style={{ color: "var(--text-muted)" }}>
              <Search className="w-5 h-5" />
            </button>
            <ThemeToggle />
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 rounded-lg touch" style={{ color: "var(--text-muted)" }} aria-label="Menu">
      ...
      {isMenuOpen && (
        <div className="lg:hidden mobile-nav-enter" style={{ borderTop: "1px solid var(--border)", background: "rgba(13,13,20,0.98)" }}>
